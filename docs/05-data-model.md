...
| `lineups`                                          | Every manager's XI, per match                             |
...
| Item                          | Status                                                                                                                                                                                                                                                                                                                                                                                                               |
| ----------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Sparse lineups and squads** | **Match-based leagues only.** Their lineups are one entry per match, with the same value copied forward, and since a lineup applies forward until changed, most of those copies carry no information. Writing only on change would cut the leaderboard read by roughly 8×. **Deferred because it only matters at 60+ match tournaments**, and Phase 1 runs on short series. Structure and field names would be identical; only the number of entries changes. **Gameweek-based leagues are not affected** — they store one lineup per gameweek and were never dense. Squads are match-keyed in both. |
