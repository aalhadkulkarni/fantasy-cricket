...
| Item                          | Status                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| ----------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Sparse lineups and squads** | **`matchBasedLineups` only.** Those are one entry per match, with the same value copied forward, and since a lineup applies forward until changed, most of those copies carry no information. Writing only on change would cut the leaderboard read by roughly 8×. **Deferred because it only matters at 60+ match tournaments**, and Phase 1 runs on short series. Structure and field names would be identical; only the number of entries changes. **`gameWeekBasedLineups` is copied forward the same way** — saving a gameweek writes it to every later gameweek — but at one entry per gameweek it is small enough not to matter. Squads are match-keyed in both. |
